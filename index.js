...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=559288558755:+55 92 8855-8765\n' // WhatsApp ID + phone number
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adeus, @${num.split('@')[0]} você não vai fazer falta aqui ghost 🤬`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Tá processando em quanto isso vai tmnc❤️😇',
				success: '✔️ Sucesso ✔️',
			...
			const ownerNumber = ["559185764773@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '559185764773@s.whatsapp.net'
                        /******ApiKey Input******/
